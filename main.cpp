...
double duration_sec(const high_resolution_clock::time_point &start,
...
struct Configuration {
  ...
  // we only break if we find a good enough cut that is also this balanced (has
  // this minside volume)
...
struct Logger {
  ...
  Logger() : nul(){};
  decltype(cout) &progress() { return silent ? nul : cout; };
  decltype(cout) &debug() { return verbose ? cout : nul; };
} l;
...
struct CutStats {
 ...
 public:
  ...
  long minside_volume() { return is_min_side ? cut_volume : noncut_volume(); }

  long maxside_volume() { return is_min_side ? noncut_volume() : cut_volume; }

  size_t diff() { return max_side - min_side; }

  size_t num_vertices() { return min_side + max_side; }

  double imbalance() { return diff() * 1. / num_vertices(); }
  ...
  void print() {
    ...
    cout << "cut size: (" << min_side << " | " << max_side << ")" << endl
         << "diff: " << diff() << " (" << imbalance() << " of total n vertices)"
         << endl;
...
// already earlier.
static void parse_chaco_format(const string &filename, ListGraph &g,
                               vector<Node> &nodes) {
  assert(nodes.empty());
...
void initGraph(GraphContext &gc, InputConfiguration config) {
  ...
  for (NodeIt n(gc.g); n != INVALID; ++n) {
    for (IncEdgeIt e(gc.g, n); e != INVALID; ++e) gc.orig_degree[n] += 1;
  }
...
// correct tho
inline int flow(const ArcLookUp<G> &alp,
                const unique_ptr<Preflow<G, EdgeMapi>> &f, Node u, Node v) {
...
void createSubdividedGraph(SubdividedGraphContext &sgc) {
  ...
  vector<Edge> edges;
  for (EdgeIt e(g); e != INVALID; ++e) {
    if (g.u(e) != g.v(e)) edges.push_back(e);
  }
...
struct CutMatching {
  ...
  // During the matching step a lot of local setup is actually made, so it makes
  // sense to group it inside a "matching context" that exists for the duration
  // of the mathing step
  ...
  struct MatchResult {
    Cutp cut_from_flow;
    size_t
        capacity;  // First capacity (minumum) that worked to get full flow thru
  };

  inline void extract_path_fast(const G &g,
  ...
  void decompose_paths_fast(const MatchingContext &mg,
  ...
  MatchResult bin_search_flows(MatchingContext &mg, unique_ptr<FlowAlgo> &p,
    ...
    for (; cap < flow_target * 2; cap *= 2) {
      ...
      if (reachedFullFlow)
        l.debug() << "We have achieved full flow, but half this capacity "
                     "didn't manage that!"
                  << endl;
      ...
      // In this case this was never done even once, so we have to do it before
      // breaking
    ...
    auto stop = now();
    l.progress() << "Flow search took (seconds) " << duration_sec(start, stop)
                 << endl;

    return result;
  }

  void decompose_paths(const MatchingContext &mg, const unique_ptr<FlowAlgo> &p,
                       vector<array<Node, 2>> &paths) {
    decompose_paths_fast(mg, p, paths);
  }

  template <typename GG>
  void make_sink_source(GG &g, MatchingContext &mg,
                        const set<Node> &cut) const {
    mg.s = g.addNode();
    mg.t = g.addNode();
    int s_added = 0;
    int t_added = 0;
  ...
  Bisectionp cut_player(const GG &g,
                        const vector<unique_ptr<M>> &given_matchings,
  ...
  // TODO Let listedgeset just be 2-arrays of nodes. Lemon is getting in the way
  // too much. But also what is assigned in MtchResult?
  MatchResult matching_player(const set<Node> &bisection, Matching &m_out) {
    ...
    // In this version, in one run of the matching the cut is strictly decided.
    // We just need to decide which one of them. Only when we change to edge
    // will the cut need to be explicitly extracted. Rn the important thing is
    // to save cuts between rounds so I can choose the best.
  ...
  // TODO Let listedgeset just be 2-arrays of nodes. Lemon is getting in the way
  // too much. But also what is assigned in MtchResult?
  MatchResult sub_matching_player(const set<Node> &bisection,
  ...
  // Theres a lot of risk for problems with "is this a cut in the orig graph or
  in the splits? unique_ptr<RoundReport> one_round() { unique_ptr<RoundReport>
  report = make_unique<RoundReport>();
...
*/
  ...
  unique_ptr<RoundReport> sub_one_round() {
    ...
    // h_multi_out_sub);
    Bisectionp sub_bisection =
        cut_player(sgc.only_splits, sub_matchings, report->multi_h_conductance);
    ...
    return move(report);
    // Thing is, the cut is not a "materialized cut" in the subdiv graph. But
    // the stats we want are the implied cut on the orig graph.
  }
  ...
  config.H_phi_target) { cout << "H Expansion target reached, this will be case
  1 or 3. According to theory, this means we probably won't find a better cut.
  That is, assuming you set H_phi right. " "If was used together with G_phi
  target, this also certifies the input graph is a G_phi expander unless there
  was a very unbaanced cut somewhere, which we will proceed to look for." <<
  endl; reached_H_target = true; return true;
      }
  ...
  relatively balanced. Cut-matching game has found a balanced cut as good as you
  wanted it."
                   << endl;
              return true;
          }
          ...
          if(!config.use_volume_treshold) {
              cout << "G Expansion target reached. Cut-matching game has found a
  cut as good as you wanted it. Whether it is balanced or not is up to you."
                   << endl;
              return true;
          }
      }
  ...
  bool sub_should_stop() {
    ...
    if (config.use_H_phi_target &&
      ...
      cout << "H Conductance target reached, this will be case 1 or 3. "
              "According to theory, this means we probably won't find a better "
              "cut. That is, assuming you set H_phi right. "
              "If was used together with G_phi target, this also certifies the "
    ...
    if ((*last_round->cut).size() > 0 &&
      ...
      if (last_round->relatively_balanced) {
        cout << "CASE2 G Expansion target reached with a cut that is "
                "relatively balanced. Cut-matching game has found a balanced "
...
cxxopts::Options create_options() {
  ...
  options.add_options()("h,help", "Show help")(
      ...
      cxxopts::value<std::string>())(
      "n,nodes",
      "Number of nodes in graph to generate. Should be even. Ignored if -f is "
      "set.",
...
void parse_options(int argc, char **argv, Configuration &config) {
  ...
  if (result.count("nodes")) assert(!config.input.load_from_file);
  ...
  if (result.count("verbose")) l.verbose = result["verbose"].as<bool>();
  if (result.count("Silent")) l.silent = result["Silent"].as<bool>();
...
set<Node> cut_complement(vector<Node> nodes, set<Node> cut) {
  set<Node> R;
...
ListDigraph *digraph_from_graph(G &g, DG &dg) {
  for (NodeIt n(g); n != INVALID; ++n) dg.addNode();
...
set<DGNode> level_cut(DG &dg, flow_instance &fp,
  ...
  if (n_upper_nodes == 0) return trimmed_nodes;
...
unit_start:
  ...
  while (fp.active_nodes.size() > 0) {
    ...
    for (DGOutArcIt e(dg, n); e != INVALID; ++e) {
      if (dg.source(e) == dg.target(e)) continue;
      if (!fp.A.count(dg.source(e)) || !fp.A.count(dg.target(e))) {
      ...
      if (fp.node_label[n] - fp.node_label[(dg.target(e))] == 1 &&
        ...
        double deg_min_ex =
            fp.node_cap[n] * fp.phi / 2.0 - (fp.node_cap[n] - fp.node_flow[n]);
        ...
        if (fp.node_flow[dg.target(e)] -
            ...
            fp.node_flow[dg.target(e)] - phi <= fp.node_cap[dg.target(e)]) {
    ...
    // relabel
    for (DGOutArcIt e(dg, n); e != INVALID; ++e) {
      if (n == dg.target(e) || !fp.A.count(dg.target(e))) continue;
      assert(n == dg.source(e));
    ...
    if (fp.node_label[n] >= fp.h - 1) fp.active_nodes.erase(n);
    goto unit_start;
  }
  ...
  if (cut_size_before == fp.A.size()) return;
...
set<Node> slow_trimming(GraphContext &gc, Configuration conf, set<Node> cut,
                        double phi) {
...
set<Node> trimming(GraphContext &gc, Configuration conf, set<Node> cut,
                   double phi) {
  // assert (phi >= 0.00001);
  assert(cut.size() >= gc.nodes.size() / 2);
  DG dg;
  ...
  for (auto &n : cut) A.insert(dg.nodeFromId(gc.g.id(n)));

  std::set_difference(gc.nodes.begin(), gc.nodes.end(), cut.begin(), cut.end(),
                      std::inserter(R_ug, R_ug.end()));
  // cut.swap(R_ug);

  for (auto &n : R_ug) R.insert(dg.nodeFromId(gc.g.id(n)));
  ...
  // dummy
  cut.clear();
  for (auto &n : fp.A) cut.insert(gc.g.nodeFromId(dg.id(n)));
  return cut;
}
...
void graph_from_cut(GraphContext &g, GraphContext &sg, set<Node> cut) {
  ...
  for (const auto &n : cut) {
    for (IncEdgeIt a(g.g, n); a != INVALID; ++a) {
      ...
      if (cut.count(g.g.target(a)) > 0 && cut.count(g.g.source(a)) > 0 &&
          g.g.source(a) <
              g.g.target(a)) {  // findEdge(sg.g, reverse_map[g.g.source(a)],
...
map<Node, Node> graph_from_cut(GraphContext &g, GraphContext &sg, set<Node> cut,
                               map<Node, Node> old_map,
...
vector<set<Node>> find_connected_components(GraphContext &g) {
  ...
  // Q:
  int test_check = 0;
  for (NodeIt n(g.g); n != INVALID; ++n) test_check = test_check + 1;
...
return_labels:
  int tot = 0;
  for (auto &l : labels) tot = tot + l.size();
...
set<Node> connected_component_from_cut(GraphContext &gc_orig, set<Node> A) {
  ...
  for (auto &n : gc_orig.nodes) gc_to_gc[n] = n;
...
set<Node> cut_from_cm(GraphContext &gc, Configuration config) {
  ...
  default_random_engine random_engine =
      config.seed_randomness ? default_random_engine(config.seed)
...
bool test_subgraph_expansion(GraphContext &gc, Configuration config,
                             set<Node> cut, double acceptance_ratio) {
  cm_result cm_g;
  cm_result cm_sg;
...
vector<map<Node, Node>> decomp(
  ...
  /*
   auto start2 = now();
   p->runMinCut(); // Note that "startSecondPhase" must be run to get flows for
                   // individual verts
   auto stop2 = now();
   l.progress() << "flow: " << p->flowValue() << " ("
                << duration_sec(start2, stop2) << " s)" << endl;
...
#pragma omp parallel for num_threads(t) schedule(dynamic, 1)
    for (int i = 0; i < 2; i++) {
      ...
      map<Node, Node> new_map =
          graph_from_cut(gc, A, cut, map_to_original_graph, i == 1);
...
#pragma omp critical
  ...
  // check best cut found so far
    ...
    auto start2 = now();
    set<Node> real_trim_cut =
        trimming(gc, config, cut_saved, config.G_phi_target);
    auto stop2 = now();
    ...
    GraphContext A;
    map<Node, Node> R_map =
        graph_from_cut(gc, V_over_A, cut, map_to_original_graph, true);
    map<Node, Node> A_map =
        graph_from_cut(gc, A, cut, map_to_original_graph, false);
...
void butterfly_test(Configuration config, int ls, int rs, int conn_ls,
                    int conn_rs) {
  GraphContext gc;
  ...
  set<Node> new_cut =
      slow_trimming(gc, config, cut, 1. / pow(gc.nodes.size(), 2));
  GraphContext sg_2;
...
void test_connect_subgraph(GraphContext &gc, Configuration conf, double phi) {
  GraphContext sg;
...
void test_expander(GraphContext &gc, Configuration conf, double phi) {
  ...
  int h = (double(gc.num_edges) /
           (10 * conf.volume_treshold_factor * pow(log2(gc.num_edges), 2)));
  ...
  if (nodes_to_remove == 0) {
    cout << "h = 0, setting to 5" << endl;
    nodes_to_remove = 5;
  }
  cout << "removing n nodes: " << nodes_to_remove << endl;

  for (int i = 0; i < gc.nodes.size(); i++) indices.push_back(i);
  shuffle(indices.begin(), indices.end(),
          default_random_engine(random_device()()));
  indices.erase(indices.begin(),
  ...
  cout << "before trimming, graph with nodes removed has n components: "
       << cc_.size() << endl;
  cout << "and n nodes:" << gc_nodes_removed.nodes.size() << endl;
  GraphContext gc_nodes_removed_complement;
  graph_from_cut(gc, gc_nodes_removed_complement,
                 cut_complement(gc.nodes, cut));
  cc_ = find_connected_components(gc_nodes_removed_complement);
  cout << "complement graph with nodes removed has n components: " << cc_.size()
       << endl;
  ...
  if (true) {
      ...
      set<Node> trim_cut = slow_trimming(gc, conf, cut,
  cm_res.best_conductance);
      //Q: we might return a really small (<<< original nodes / 2) cut in one
  round
      //trim_cut = slow_trimming(gc, conf, trim_cut, cm_res.best_conductance);
      GraphContext slow_trim_gc;
      GraphContext slow_trim_gc_complement;
      graph_from_cut(gc, slow_trim_gc, trim_cut);
      graph_from_cut(gc, slow_trim_gc_complement, cut_complement(gc.nodes,
  trim_cut)); vector<set<Node>> cc = find_connected_components(slow_trim_gc);
      cout << "after slow trim, induced subgraph has n components: " <<
  cc.size() << endl; cc = find_connected_components(slow_trim_gc_complement);
      cout << "after slow trim, induced subgraph has n components: " <<
  cc.size() << endl; assert (connected(slow_trim_gc.g) && "Slow trimming must
  yield connected subgraph"); set<Node> fast_trim_cut = trimming(gc, conf, cut,
  conf.G_phi_target); GraphContext fast_trim_gc; graph_from_cut(gc,
  fast_trim_gc, fast_trim_cut); vector<set<Node>> cc_ =
  find_connected_components(fast_trim_gc); cout << "after fast trim, subgraph
  has n components: " << cc_.size() << endl; GraphContext
  fast_trim_gc_complement; graph_from_cut(gc, fast_trim_gc,
  cut_complement(gc.nodes, fast_trim_cut)); cc_ =
  find_connected_components(fast_trim_gc_complement); cout << "after fast trim,
  COMPLEMENT subgraph has n components: " << cc_.size() << endl; assert
  (connected(fast_trim_gc.g) && "Real trimming must yield connected subgraph");
      cut = fast_trim_cut;
  }
  */
  ...
  for (NodeIt n(gc.g); n != INVALID; ++n) dummy_map_2[n] = n;
  ...
  assert("Trimming guarantee failed" &&
         (cm_res_after_trimming.best_cut.size() == 0 ||
          cm_res_after_trimming.best_conductance > conf.G_phi_target / 6));

  /*
  cout << "EXPANDER: local flow: old cut size: " << cut.size() << " new cut
  size: " << new_cut.size() << endl;

  new_cut = slow_trimming(gc, conf, cut, phi);

  cout << "EXPANDER EXACT: local flow: old cut size: " << cut.size() << " new
  cut size: " << new_cut.size() << endl;
  */
  return;
}
...
int main(int argc, char **argv) {
  ...
  cout << "n: gc.g.nodes.size()"
       << " e: " << gc.num_edges << endl;
  map<Node, Node> map_to_original_graph;
  for (NodeIt n(gc.g); n != INVALID; ++n) map_to_original_graph[n] = n;
  ...
  for (NodeIt n(gc.g); n != INVALID; ++n) {
    int d = 0;
    for (IncEdgeIt e(gc.g, n); e != INVALID; ++e) d++;
    degree[n] = d;
  }
  ...
  for (const auto &m : cut_maps) {
    if (m.size() == 1) {
      for (const auto &c : m) {
        assert(count(all_nodes.begin(), all_nodes.end(), gc.g.id(c.second)) ==
               0);
  ...
  for (int i = 0; i < cuts_node_vector.size(); ++i) {
    ...
    for (const auto &n : cuts_node_vector[i]) {
      for (IncEdgeIt e(gc.g, n); e != INVALID; ++e) {
        ...
        if (count(cuts_node_vector[i].begin(), cuts_node_vector[i].end(),
                  gc.g.target(e)) > 0) {
